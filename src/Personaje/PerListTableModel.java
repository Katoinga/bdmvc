...
public class PerListTableModel extends AbstractTableModel {
	...
	String pass="";
    CachedRowSet PerListRowSet; // Contains data
    ...
    public PerListTableModel() {
        ...
        try {
            ...
            PerListRowSet = new CachedRowSetImpl();
            PerListRowSet.setType(ResultSet.TYPE_SCROLL_INSENSITIVE);
            PerListRowSet.setConcurrency(ResultSet.CONCUR_UPDATABLE);
            PerListRowSet.setCommand("SELECT * FROM personaje");
            PerListRowSet.execute(connection);
            
            metadata = PerListRowSet.getMetaData();
            numcols = metadata.getColumnCount();
            numrows = PerListRowSet.size();
            PerListRowSet.first();
    ...
    public PerListTableModel(RowSet rowSet, Connection conn) {
        try {
            PerListRowSet = (CachedRowSet) rowSet;
            PerListRowSet.setType(ResultSet.TYPE_SCROLL_INSENSITIVE);
            PerListRowSet.setConcurrency(ResultSet.CONCUR_UPDATABLE);
            metadata = PerListRowSet.getMetaData();
            numcols = metadata.getColumnCount();
    ...
    public Object getValueAt(int row, int col) {
        try {
            if (row >= PerListRowSet.size())
                //Error: Trying to access a deleted row
                return null;

            PerListRowSet.absolute(row + 1);
            Object obj = PerListRowSet.getObject(col + 1);
            if (obj == null)
                return null;
            else
                return obj.toString();
    ...
    public void setValueAt(Object aValue, int row, int col) {
        try {
            PerListRowSet.moveToInsertRow();
            System.out.println(aValue + " setValueAt " + (row+1) + " " + (col+1));
            PerListRowSet.updateObject(col+1, (String) aValue);
        } catch (SQLException err) {
            err.getMessage();
            err.printStackTrace();
        }
    ...
    public boolean isPrimaryKeyInDB(Object[] array) {
        try {
            ...
            PerListRowSet.beforeFirst();
            while (PerListRowSet.next()) {
                if (PerListRowSet.getString("PerCod").equals(array[1]))
    ...
    public void addRow(Object[] array) {
        try {
            ...
            if (!isPrimaryKeyInDB(array)) {
                PerListRowSet.last();
                PerListRowSet.moveToInsertRow();
                PerListRowSet.updateString("PerNom", (String) array[0]);
                PerListRowSet.updateString("PerCod", (String) array[1]);
                PerListRowSet.updateString("PerNac", (String) array[2]);
                PerListRowSet.updateString("PerNacCod", (String) array[3]);
                PerListRowSet.updateString("PerSexCod", (String) array[4]);

                PerListRowSet.updateString("PerEstReg", (String) array[5]);

                PerListRowSet.insertRow();
                PerListRowSet.moveToCurrentRow();
                PerListRowSet.acceptChanges(connection);
            } else {
                // Error: User's desired primary key is already in the database
                JOptionPane.showMessageDialog(null,
                        "Cannot have multiple records with the same primary key (PerCod).",
    ...
    public void deleteRow(Object[] array) {
        try {
            ...
            while (PerListRowSet.next()) {
                if (PerListRowSet.getString("PerCod").equals(array[1])) {
                	PerListRowSet.updateString("PerEstReg", "*");
                    
                    PerListRowSet.updateRow();
                    PerListRowSet.first();
                    break;
                }
            }
    ...
    public void updateRow(Object[] array, JTable jtable) {
        try {
            ...
            PerListRowSet.absolute(jtable.getSelectedRow() + 1);

            // Current Per number in the selected row
            String selectedPerNumber = PerListRowSet.getString("PerCod");

            // Per number entered into the text field
            String desiredPerNumber = (String) array[1];
            ...
            if (!isPrimaryKeyInDB(array) || desiredPerNumber.equals(selectedPerNumber)) {
                PerListRowSet.absolute(jtable.getSelectedRow() + 1);
                PerListRowSet.updateString("PerNom", (String) array[0]);
                PerListRowSet.updateString("PerCod", (String) array[1]);
                PerListRowSet.updateString("PerNac", (String) array[2]);
    ...
    public void activateRow(Object[] array) {
        try {
            ...
            while (PerListRowSet.next()) {
                if (PerListRowSet.getString("PerCod").equals(array[1])) {
                	PerListRowSet.updateString("PerEstReg", "A");
    ...
    public void inactivateRow(Object[] array) {
        try {
            ...
            while (PerListRowSet.next()) {
                if (PerListRowSet.getString("PerCod").equals(array[1])) {
                	PerListRowSet.updateString("PerEstReg", "I");
    ...
    public static void main(String[] args) {
        ...
        PerListGUI PerListGUI = new PerListGUI();
        PerListGUI.setVisible(true);
    }
