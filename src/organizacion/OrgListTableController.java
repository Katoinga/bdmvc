...
public class OrgListTableController implements ListSelectionListener, RowSetListener {
    
    private OrgListTableModel tableModel = null;
    private OrgListGUI gui;
    
    public OrgListTableController(OrgListGUI gui) {
        this.gui = gui;
        // Create the tableModel using the data in the cachedRowSet
        tableModel = new OrgListTableModel();
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        if (!e.getValueIsAdjusting()) {
            ...
            gui.setGenEstRegTextField((String) tableModel.getValueAt(firstIndex, 0));
        }
    }

    // Add a row to the database using addRow() in OrgListTableModel
    public void addRow(String[] array) {
        tableModel.addRow(array);
    }

    // Delete a row to the database using deleteRow() in OrgListTableModel
    public void deleteRow(String[] array) {
        tableModel.deleteRow(array);
    }

    // Update a row to the database using updateRow() in OrgListTableModel
    public void updateRow(String[] array, JTable jtable) {
        tableModel.updateRow(array, jtable);
    }
    
    // Activate a row to the database using updateRow() in OrgListTableModel
    public void activateRow(String[] array) {
        tableModel.activateRow(array);
    }
    
    // Inactivate a row to the database using updateRow() in OrgListTableModel
    ...
    public void rowChanged(RowSetEvent event) {
        try {
            ...
            // getRowSet() is a method in OrgListTableModel that returns a CachedRowSet object
            // getRow() is a method in ResultSet that returns the current row
            tableModel.getRowSet().moveToCurrentRow();
            int firstIndex = tableModel.getRowSet().getRow();

            // Create a new table model with the new data
            tableModel = new OrgListTableModel(tableModel.getRowSet(), tableModel.getConnection());
            ...
            gui.setGenNameTextField((String) tableModel.getValueAt(firstIndex, 1));
            gui.setGenNumberTextField((String) tableModel.getValueAt(firstIndex, 0));
            gui.setGenEstRegTextField((String) tableModel.getValueAt(firstIndex, 2));
        } catch (Exception exp) {
            exp.getMessage();
            exp.printStackTrace();
        }
