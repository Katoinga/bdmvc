...
public class SalListTableController implements ListSelectionListener, RowSetListener {
    
    private SalListTableModel tableModel = null;
    private SalListGUI gui;
    
    public SalListTableController(SalListGUI gui) {
        this.gui = gui;
        // Create the tableModel using the data in the cachedRowSet
        tableModel = new SalListTableModel();
    ...
    public void valueChanged(ListSelectionEvent e) {
        // Salvent action from being Salformed twice
        ...
        if (!e.getValueIsAdjusting()) {
            ...
            gui.setSalNumberTextField((String) tableModel.getValueAt(firstIndex, 0));
            gui.setSalAfoTextField((String) tableModel.getValueAt(firstIndex, 1));
            gui.setSalCinCodComboBox((String) tableModel.getValueAt(firstIndex, 2));      
            gui.setSalEstRegTextField((String) tableModel.getValueAt(firstIndex, 3));

        }
    }

    // Add a row to the database using addRow() in SalListTableModel
    public void addRow(String[] array) {
        tableModel.addRow(array);
    }

    // Delete a row to the database using deleteRow() in SalListTableModel
    public void deleteRow(String[] array) {
        tableModel.deleteRow(array);
    }

    // Update a row to the database using updateRow() in SalListTableModel
    public void updateRow(String[] array, JTable jtable) {
        tableModel.updateRow(array, jtable);
    }
    
    // Activate a row to the database using updateRow() in SalListTableModel
    public void activateRow(String[] array) {
        tableModel.activateRow(array);
    }
    
    // inactivate a row to the database using updateRow() in SalListTableModel
    ...
    public void rowChanged(RowSetEvent event) {
        try {
            ...
            // getRowSet() is a method in SalListTableModel that returns a CachedRowSet object
            // getRow() is a method in ResultSet that returns the current row
            tableModel.getRowSet().moveToCurrentRow();
            int firstIndex = tableModel.getRowSet().getRow();

            // Create a new table model with the new data
            tableModel = new SalListTableModel(tableModel.getRowSet(), tableModel.getConnection());
