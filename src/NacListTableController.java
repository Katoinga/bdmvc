...
public class NacListTableController implements ListSelectionListener, RowSetListener {
    
    private NacListTableModel tableModel = null;
    private NacListGUI gui;
    
    public NacListTableController(NacListGUI gui) {
        this.gui = gui;
        // Create the tableModel using the data in the cachedRowSet
        tableModel = new NacListTableModel();
    ...
    public void valueChanged(ListSelectionEvent e) {
        ...
        if (!e.getValueIsAdjusting()) {
            ...
            gui.setNacNameTextField((String) tableModel.getValueAt(firstIndex, 1));
            gui.setNacNumberTextField((String) tableModel.getValueAt(firstIndex, 0));
            gui.setNacEstRegTextField((String) tableModel.getValueAt(firstIndex, 2));

        }
    }

    // Add a row to the database using addRow() in NacListTableModel
    public void addRow(String[] array) {
        tableModel.addRow(array);
    }

    // Delete a row to the database using deleteRow() in NacListTableModel
    public void deleteRow(String[] array) {
        tableModel.deleteRow(array);
    }

    // Update a row to the database using updateRow() in NacListTableModel
    public void updateRow(String[] array, JTable jtable) {
        tableModel.updateRow(array, jtable);
    }
    
    // Activate a row to the database using updateRow() in NacListTableModel
    public void activateRow(String[] array) {
        tableModel.activateRow(array);
    }
    
    // Inactivate a row to the database using updateRow() in NacListTableModel
    ...
    public void rowChanged(RowSetEvent event) {
        try {
            ...
            // getRowSet() is a method in NacListTableModel that returns a CachedRowSet object
            // getRow() is a method in ResultSet that returns the current row
            tableModel.getRowSet().moveToCurrentRow();
            int firstIndex = tableModel.getRowSet().getRow();

            // Create a new table model with the new data
            tableModel = new NacListTableModel(tableModel.getRowSet(), tableModel.getConnection());
