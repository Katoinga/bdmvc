...
public class PreListTableController implements ListSelectionListener, RowSetListener {
    
    private PreListTableModel tableModel = null;
    private PreListGUI gui;
    
    public PreListTableController(PreListGUI gui) {
        this.gui = gui;
        // Create the tableModel using the data in the cachedRowSet
        tableModel = new PreListTableModel();
    ...
    public void valueChanged(ListSelectionEvent e) {
        // Prevent action from being Preformed twice
        ...
        if (!e.getValueIsAdjusting()) {
            ...
            gui.setPreNameTextField((String) tableModel.getValueAt(firstIndex, 1));
            gui.setPreNumberTextField((String) tableModel.getValueAt(firstIndex, 0));
            gui.setPreTarCodComboBox((String) tableModel.getValueAt(firstIndex, 2));
            gui.setPreEstRegTextField((String) tableModel.getValueAt(firstIndex, 3));

        }
    }

    // Add a row to the database using addRow() in PreListTableModel
    public void addRow(String[] array) {
        tableModel.addRow(array);
    }

    // Delete a row to the database using deleteRow() in PreListTableModel
    public void deleteRow(String[] array) {
        tableModel.deleteRow(array);
    }

    // Update a row to the database using updateRow() in PreListTableModel
    public void updateRow(String[] array, JTable jtable) {
        tableModel.updateRow(array, jtable);
    }
    
    // Activate a row to the database using updateRow() in PreListTableModel
    public void activateRow(String[] array) {
        tableModel.activateRow(array);
    }
    
    // inactivate a row to the database using updateRow() in PreListTableModel
    ...
    public void rowChanged(RowSetEvent event) {
        try {
            ...
            // getRowSet() is a method in PreListTableModel that returns a CachedRowSet object
            // getRow() is a method in ResultSet that returns the current row
            tableModel.getRowSet().moveToCurrentRow();
            int firstIndex = tableModel.getRowSet().getRow();

            // Create a new table model with the new data
            tableModel = new PreListTableModel(tableModel.getRowSet(), tableModel.getConnection());
